    ...
    <template>
      ...
      <div class="transfer-left">
        ...
        <h3 class="transfer-title">
          <el-checkbox :indeterminate="from_is_indeterminate"
        ...
        <div class="transfer-main">
          ...
          <slot name="from"></slot>
          <el-input v-if="filter" :placeholder="placeholder"
            v-model="filterFrom"
            size="small"
            class="filter-tree"
          ></el-input>

          <el-tree ref="from-tree" show-checkbox
            :lazy="lazy"
            :node-key="node_key"
            :load="leftloadNode"
            :props="defaultProps"
...
<script>
import { arrayToTree } from '../common/utils'
export default {
  ...
  methods: {
    ...
    // 添加按钮
    addToAims (emit) {
      // 获取选中通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信 选中方法getCheckedKeys()
      let keys = this.$refs['from-tree'].getCheckedKeys()
      // 获取半选通过穿梭框的keys - 仅用于传送纯净的id数组到父组件同后台通信 半选方法getHalfCheckedKeys()
      let harfKeys = this.$refs['from-tree'].getHalfCheckedKeys()
      // 选中节点数据 getCheckedNodes()
